...
#include "stdafx.h"
#include "sample_hk_login.h"
#include "sample_hk_loginDlg.h"
...
Csample_hk_loginDlg::Csample_hk_loginDlg(CWnd* pParent /*=NULL*/)
  : CDialogEx(Csample_hk_loginDlg::IDD, pParent)
  , m_bIsExiting(false)
  , m_HKLoginIng(false)
  , m_dwHKErrCode(0)
  , m_HKLoginID(-1)
...
void Csample_hk_loginDlg::DoDataExchange(CDataExchange* pDX)
{
	DDX_Control(pDX, IDC_HOME_WEBSITE, m_ctrlHome);
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(Csample_hk_loginDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_MESSAGE(WM_DEVICE_LOGIN_SUCCESS, &Csample_hk_loginDlg::OnDeviceLoginSuccess)
	ON_BN_CLICKED(IDC_BUTTON_ASYNC, &Csample_hk_loginDlg::OnBnClickedButtonAsync)
	ON_BN_CLICKED(IDC_BUTTON_LOGOUT, &Csample_hk_loginDlg::OnBnClickedButtonLogout)
END_MESSAGE_MAP()
...
void Csample_hk_loginDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void Csample_hk_loginDlg::OnPaint()
{
	if (IsIconic())
	{
...
HCURSOR Csample_hk_loginDlg::OnQueryDragIcon()
{
...
//
// 执行DVR登录操作...
DWORD Csample_hk_loginDlg::doDeviceLogin(LPCTSTR lpIPAddr, int nCmdPort, LPCTSTR lpUser, LPCTSTR lpPass)
{
	// 将摄像机的错误标志复位...
...
//
// 登录回调函数接口...
void CALLBACK Csample_hk_loginDlg::DeviceLoginResult(LONG lUserID, DWORD dwResult, LPNET_DVR_DEVICEINFO_V30 lpDeviceInfo, void * pUser)
{
...
//
// 实际处理登录回调的函数...
void Csample_hk_loginDlg::onDeviceLoginAsync(LONG lUserID, DWORD dwResult, LPNET_DVR_DEVICEINFO_V30 lpDeviceInfo)
{
	// 正在登录状态复位...
	m_HKLoginIng = false;
	// 处在退出状态中，直接返回...
...
//
// 设备异常回调函数接口...
void CALLBACK Csample_hk_loginDlg::DeviceException(DWORD dwType, LONG lUserID, LONG lHandle, void * pUser)
{
	Csample_hk_loginDlg * lpDlg = (Csample_hk_loginDlg*)pUser;
